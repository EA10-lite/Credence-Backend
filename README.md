...
Invalid input returns **400** with `{ "error": "Validation failed", "details": [{ "path", "message" }] }`. See [docs/VALIDATION.md](docs/VALIDATION.md). Rate limiting returns **429** with `Retry-After`; see [docs/rate-limiting.md](docs/rate-limiting.md).
